...
int receive_tar_file(char *directory_path) {
    char tmpfile[1024];
    ...
    // 检查磁盘空间是否满足要求
    if (is_directory_full(directory_path) != 0) {
    ...
    time(&t);
    t += 8 * 3600; // 加上8小时
    tm_info = gmtime(&t); // 使用gmtime而不是localtime
...
int response_handle(char buffer[]) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        while (resp_ret == 0) {
            ...
            resp_ret = send_message(ip_address, PORT_S1, "S1");
            sleep(1);
        }
    }
    return 0;
